...
#define SizeOfBackendParameters(startup_data_len) \
  (offsetof(BackendParameters, startup_data) + startup_data_len)

static void read_backend_variables(char* id, char** startup_data,
                                   size_t* startup_data_len);
static void restore_backend_variables(BackendParameters* param);

static bool save_backend_variables(BackendParameters* param,
                                   ClientSocket* client_sock,
#ifdef WIN32
 ...
 * the child process.
 */
pid_t postmaster_child_launch(BackendType child_type, char* startup_data,
                              size_t startup_data_len,
                              ClientSocket* client_sock) {
  pid_t pid;
...
#ifdef EXEC_BACKEND
  pid = internal_forkexec(child_process_kinds[child_type].name, startup_data,
  ...
  if (pid == 0) /* child */
  {
     ...
     */
    ereport(LOG, (errmsg("launching %s process",
                         child_process_kinds[child_type].name)));
    child_process_kinds[child_type].main_fn(startup_data, startup_data_len);
    ereport(LOG, (errmsg("launching %s process finished",
                         child_process_kinds[child_type].name)));
...
static pid_t internal_forkexec(const char* child_kind, char* startup_data,
  ...
  if (!save_backend_variables(param, client_sock, startup_data,
                              startup_data_len)) {
    pfree(param);
...
retry:
  ...
  if (!CreateProcess(NULL, cmdLine, NULL, NULL, TRUE, CREATE_SUSPENDED, NULL,
                     NULL, &si, &pi)) {
    ereport(LOG, (errmsg("CreateProcess() call failed: %m (error code %lu)",
                         GetLastError())));
    UnmapViewOfFile(param);
    CloseHandle(paramHandle);
    return -1;
  ...
  if (!pgwin32_ReserveSharedMemoryRegion(pi.hProcess)) {
    ...
    if (!TerminateProcess(pi.hProcess, 255))
      ereport(LOG, (errmsg_internal("could not terminate process that failed "
                                    "to reserve memory: error code %lu",
                                    GetLastError())));
    CloseHandle(pi.hProcess);
    CloseHandle(pi.hThread);
    if (++retry_count < 100)
 ...
 *then dispatch to the appropriate place.
 *
...
static bool save_backend_variables(BackendParameters* param,
  ...
  if (!write_inheritable_socket(
          &param->inh_sock, client_sock ? client_sock->sock : PGINVALID_SOCKET,
          childPid))
    return false;
...
static bool write_duplicated_handle(HANDLE* dest, HANDLE src,
  ...
  if (!DuplicateHandle(GetCurrentProcess(), src, childProcess, &hChild, 0, TRUE,
    ...
    ereport(LOG, (errmsg_internal("could not duplicate handle to be written to "
                                  "backend parameter file: error code %lu",
                                  GetLastError())));
    return false;
  }
  ...
  if (param.startup_data_len > 0) {
    *startup_data = palloc(*startup_data_len);
    if (fread(*startup_data, *startup_data_len, 1, fp) != 1) {
      write_stderr(
          "could not read startup data from backend variables file \"%s\": "
          "%m\n",
          id);
      exit(1);
    }
