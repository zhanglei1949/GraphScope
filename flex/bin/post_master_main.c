...
static void init_locale(const char* categoryname, int category,
                        const char* locale);
...
int main(int argc, char* argv[]) {
   ...
   */
  printf(
      "Dispatching to one of various subprograms depending on first "
      "argument...");
...
static void init_locale(const char* categoryname, int category,
  ...
  if (pg_perm_setlocale(category, locale) == NULL &&
    ...
    elog(FATAL, "could not adopt \"%s\" locale nor C locale for %s", locale,
         categoryname);
}
...
static void help(const char* progname) {
  ...
  printf(
      _("  -T                 send SIGABRT to all backend processes if one "
        "dies\n"));
  printf(
  ...
  printf(
      _("  --boot             selects bootstrapping mode (must be first "
        "argument)\n"));
  printf(
      _("  --check            selects check mode (must be first argument)\n"));
  printf(
