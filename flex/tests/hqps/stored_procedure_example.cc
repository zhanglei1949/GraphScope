...
class SampleStoredProcedure : public HqpsAppBase<gs::MutableCSRInterface> {
 public:
...
void* CreateApp(gs::GraphStoreType store_type) {
  if (store_type == gs::GraphStoreType::Grape) {
    gs::SampleStoredProcedure* app = new gs::SampleStoredProcedure();
...
void DeleteApp(void* app, gs::GraphStoreType store_type) {
  if (store_type == gs::GraphStoreType::Grape) {
    gs::SampleStoredProcedure* casted =
        static_cast<gs::SampleStoredProcedure*>(app);
    delete casted;
  }
}
}