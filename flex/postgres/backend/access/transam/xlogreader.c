 ...
 * decoded there.
 */
void XLogReaderSetDecodeBuffer(XLogReaderState* state, void* buffer,
                               size_t size) {
...
XLogReaderState* XLogReaderAllocate(int wal_segment_size, const char* waldir,
  ...
  state = (XLogReaderState*) palloc_extended(
      sizeof(XLogReaderState), MCXT_ALLOC_NO_OOM | MCXT_ALLOC_ZERO);
  if (!state)
    return NULL;
  ...
  state->errormsg_buf =
      palloc_extended(MAX_ERRORMSG_LEN + 1, MCXT_ALLOC_NO_OOM);
...
restart:
  ...
  if (targetRecOff == 0) {
    ...
    report_invalid_record(
        state, "invalid record offset at %X/%X: expected at least %u, got %u",
        LSN_FORMAT_ARGS(RecPtr), pageHeaderSize, targetRecOff);
    goto err;
  }
  ...
  if (targetRecOff <= XLOG_BLCKSZ - SizeOfXLogRecord) {
    ...
    if (total_len < SizeOfXLogRecord) {
      ...
      report_invalid_record(state,
                            "XLogDecodeNextRecord invalid record length at "
                            "%X/%X: expected at least %u, got %u",
                            LSN_FORMAT_ARGS(RecPtr), (uint32) SizeOfXLogRecord,
                            total_len);
      goto err;
    }
   ...
   */
  decoded = XLogReadRecordAlloc(state, total_len, false /* allow_oversized */);
  ...
  if (total_len > len) {
    ...
    do {
      ...
      if (pageHeader->xlp_rem_len == 0 ||
        ...
        report_invalid_record(
            state, "invalid contrecord length %u (expected %lld) at %X/%X",
            pageHeader->xlp_rem_len, ((long long) total_len) - gotlen,
            LSN_FORMAT_ARGS(RecPtr));
        goto err;
      }
    ...
    state->NextRecPtr =
        targetPagePtr + pageHeaderSize + MAXALIGN(pageHeader->xlp_rem_len);
  } else {
  ...
  if (record->xl_rmid == RM_XLOG_ID &&
    ...
    state->NextRecPtr -=
        XLogSegmentOffset(state->NextRecPtr, state->segcxt.ws_segsize);
  }
  ...
  if (decoded == NULL) {
    Assert(!nonblocking);
    decoded = XLogReadRecordAlloc(state, total_len, true /* allow_oversized */);
...
static int ReadPageInternal(XLogReaderState* state, XLogRecPtr pageptr,
  ...
  if (targetSegNo == state->seg.ws_segno && targetPageOff == state->segoff &&
   ...
   */
  readLen =
      state->routine.page_read(state, pageptr, Max(reqLen, SizeOfXLogShortPHD),
                               state->currRecPtr, state->readBuf);
  if (readLen == XLREAD_WOULDBLOCK)
    return XLREAD_WOULDBLOCK;
  else if (readLen < 0)
...
static bool ValidXLogRecordHeader(XLogReaderState* state, XLogRecPtr RecPtr,
  ...
  if (record->xl_tot_len < SizeOfXLogRecord) {
    ...
    return false;
    report_invalid_record(state,
                          "ValidXLogRecordHeader invalid record length at "
                          "%X/%X: expected at least %u, got %u",
                          LSN_FORMAT_ARGS(RecPtr), (uint32) SizeOfXLogRecord,
                          record->xl_tot_len);
  }
  ...
  if (randAccess) {
    ...
    if (!(record->xl_prev < RecPtr)) {
      report_invalid_record(
          state, "record with incorrect prev-link %X/%X at %X/%X",
          LSN_FORMAT_ARGS(record->xl_prev), LSN_FORMAT_ARGS(RecPtr));
      return false;
    }
  } else {
    /*
...
static bool ValidXLogRecord(XLogReaderState* state, XLogRecord* record,
  ...
  if (!EQ_CRC32C(record->xl_crc, crc)) {
    report_invalid_record(
        state, "incorrect resource manager data checksum in record at %X/%X",
        LSN_FORMAT_ARGS(recptr));
    return false;
  }
 ...
 * 'recptr'.
 */
bool XLogReaderValidatePageHeader(XLogReaderState* state, XLogRecPtr recptr,
                                  char* phdr) {
  XLogSegNo segno;
  int32 offset;
  ...
  if ((hdr->xlp_info & ~XLP_ALL_FLAGS) != 0) {
    ...
    report_invalid_record(
        state, "invalid info bits %04X in WAL segment %s, LSN %X/%X, offset %u",
  ...
  if (hdr->xlp_info & XLP_LONG_HEADER) {
    ...
    if (state->system_identifier &&
      ...
      report_invalid_record(state,
                            "WAL file is from different database system: WAL "
                            "file database system identifier is %llu, "
  ...
  if (hdr->xlp_pageaddr != recptr) {
    ...
    report_invalid_record(
        ...
        LSN_FORMAT_ARGS(hdr->xlp_pageaddr), fname, LSN_FORMAT_ARGS(recptr),
        offset);
    return false;
  }
  ...
  if (recptr > state->latestPagePtr) {
    if (hdr->xlp_tli < state->latestPageTLI) {
      ...
      report_invalid_record(state,
                            "out-of-sequence timeline ID %u (after %u) in WAL "
                            "segment %s, LSN %X/%X, offset %u",
  ...
  while (true) {
    ...
    if (header->xlp_info & XLP_FIRST_IS_CONTRECORD) {
      ...
      if (MAXALIGN(header->xlp_rem_len) >= (XLOG_BLCKSZ - pageHeaderSize))
         ...
         */
        tmpRecPtr =
            targetPagePtr + pageHeaderSize + MAXALIGN(header->xlp_rem_len);
        break;
      }
    } else {
 ...
 */
bool WALRead(XLogReaderState* state, char* buf, XLogRecPtr startptr, Size count,
 ...
 */
bool DecodeXLogRecord(XLogReaderState* state, DecodedXLogRecord* decoded,
...
#define COPY_HEADER_FIELD(_dst, _size) \
  ...
  while (remaining > datatotal) {
    ...
    if (block_id == XLR_BLOCK_ID_DATA_SHORT) {
      ...
      if (blk->has_data && blk->data_len == 0) {
        report_invalid_record(
            state, "BKPBLOCK_HAS_DATA set, but no data included at %X/%X",
            LSN_FORMAT_ARGS(state->ReadRecPtr));
        goto err;
      }
      if (!blk->has_data && blk->data_len != 0) {
        report_invalid_record(
            state, "BKPBLOCK_HAS_DATA not set, but data length is %u at %X/%X",
            (unsigned int) blk->data_len, LSN_FORMAT_ARGS(state->ReadRecPtr));
        goto err;
      }
      ...
      if (blk->has_image) {
        ...
        if ((blk->bimg_info & BKPIMAGE_HAS_HOLE) &&
          ...
          report_invalid_record(
              state,
              "BKPIMAGE_HAS_HOLE set, but hole offset %u length %u block image "
              "length %u at %X/%X",
              (unsigned int) blk->hole_offset, (unsigned int) blk->hole_length,
              (unsigned int) blk->bimg_len, LSN_FORMAT_ARGS(state->ReadRecPtr));
          goto err;
        }
        ...
        if (!(blk->bimg_info & BKPIMAGE_HAS_HOLE) &&
            (blk->hole_offset != 0 || blk->hole_length != 0)) {
          report_invalid_record(state,
                                "BKPIMAGE_HAS_HOLE not set, but hole offset %u "
                                "length %u at %X/%X",
         ...
         */
        if (BKPIMAGE_COMPRESSED(blk->bimg_info) && blk->bimg_len == BLCKSZ) {
          report_invalid_record(
              state,
         ...
         */
        if (!(blk->bimg_info & BKPIMAGE_HAS_HOLE) &&
            !BKPIMAGE_COMPRESSED(blk->bimg_info) && blk->bimg_len != BLCKSZ) {
          report_invalid_record(
              state,
              "neither BKPIMAGE_HAS_HOLE nor BKPIMAGE_COMPRESSED set, but "
              "block image length is %u at %X/%X",
              (unsigned int) blk->data_len, LSN_FORMAT_ARGS(state->ReadRecPtr));
          goto err;
        }
      }
      if (!(fork_flags & BKPBLOCK_SAME_REL)) {
        ...
        if (rlocator == NULL) {
          report_invalid_record(
              state, "BKPBLOCK_SAME_REL set but no previous rel at %X/%X",
              LSN_FORMAT_ARGS(state->ReadRecPtr));
          goto err;
        }
      ...
      report_invalid_record(state, "invalid block_id %u at %X/%X", block_id,
                            LSN_FORMAT_ARGS(state->ReadRecPtr));
      goto err;
    }
  }
   ...
   */
  out = ((char*) decoded) + offsetof(DecodedXLogRecord, blocks) +
 ...
 */
void XLogRecGetBlockTag(XLogReaderState* record, uint8 block_id,
                        RelFileLocator* rlocator, ForkNumber* forknum,
                        BlockNumber* blknum) {
  if (!XLogRecGetBlockTagExtended(record, block_id, rlocator, forknum, blknum,
                                  NULL)) {
#ifndef FRONTEND
...
bool RestoreBlockImage(XLogReaderState* record, uint8 block_id, char* page) {
  ...
  if (!record->record->blocks[block_id].has_image) {
    report_invalid_record(
        record, "could not restore image at %X/%X with invalid state, block %d",
        LSN_FORMAT_ARGS(record->ReadRecPtr), block_id);
    return false;
  }
  ...
  if (BKPIMAGE_COMPRESSED(bkpb->bimg_info)) {
    ...
    if ((bkpb->bimg_info & BKPIMAGE_COMPRESS_PGLZ) != 0) {
      ...
      report_invalid_record(record,
                            "could not restore image at %X/%X compressed with "
                            "%s not supported by build, block %d",
                            LSN_FORMAT_ARGS(record->ReadRecPtr), "LZ4",
                            block_id);
      return false;
#endif
    } else if ((bkpb->bimg_info & BKPIMAGE_COMPRESS_ZSTD) != 0) {
#ifdef USE_ZSTD
      size_t decomp_result = ZSTD_decompress(
          tmp.data, BLCKSZ - bkpb->hole_length, ptr, bkpb->bimg_len);
      ...
      report_invalid_record(record,
                            "could not restore image at %X/%X compressed with "
                            "%s not supported by build, block %d",
                            LSN_FORMAT_ARGS(record->ReadRecPtr), "zstd",
                            block_id);
      return false;
#endif
    } else {
