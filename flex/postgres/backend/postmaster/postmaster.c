/*
 * The customized postmaster implementation of interactive, similar to the
 * postgres postgres, but creating different backend.
 */
/*-------------------------------------------------------------------------
 *
 * postmaster.c
 ...
 *		backend screws up locks or shared memory.  Likewise, the
 *Postmaster should never block on messages from frontend clients.
 *
 * Garbage Collection:
 *		The Postmaster cleans up after backends if they have an
 *emergency exit and/or core dump.
 *
 * Error Reporting:
...
static pid_t StartupPID = 0, BgWriterPID = 0, CheckpointerPID = 0,
             WalWriterPID = 0, WalReceiverPID = 0, WalSummarizerPID = 0,
             AutoVacPID = 0, PgArchPID = 0, SysLoggerPID = 0,
             SlotSyncWorkerPID = 0, FlexPID = 0;
...
static StartupStatusEnum FlexStatus = STARTUP_NOT_RUNNING;
...
static void WINAPI pgwin32_deadchild_callback(PVOID lpParameter,
 ...
 */
void FlexPostmasterMain(int argc, char* argv[]) {
  printf("Entering FlexPostmasterMain...\n");
  int opt;
  int status;
  ...
  while ((opt = getopt(argc, argv,
    ...
    case 'T':
       ...
       */
      SetConfigOption("send_abort_for_crash", "true", PGC_POSTMASTER,
                      PGC_S_ARGV);
      break;
  ...
  if (output_config_variable != NULL) {
    ...
    if ((flags & GUC_RUNTIME_COMPUTED) == 0) {
      ...
      const char* config_val =
          GetConfigOption(output_config_variable, false, false);
  ...
  if (SuperuserReservedConnections + ReservedConnections >= MaxConnections) {
    write_stderr(
        ...
        "\"reserved_connections\" (%d) must be less than \"max_connections\" "
        "(%d)\n",
        progname, SuperuserReservedConnections, ReservedConnections,
        MaxConnections);
    ExitPostmaster(1);
  }
  if (XLogArchiveMode > ARCHIVE_MODE_OFF && wal_level == WAL_LEVEL_MINIMAL)
    ereport(ERROR, (errmsg("WAL archival cannot be enabled when \"wal_level\" "
                           "is \"minimal\"")));
  if (max_wal_senders > 0 && wal_level == WAL_LEVEL_MINIMAL)
    ereport(ERROR, (errmsg("WAL streaming (\"max_wal_senders\" > 0) requires "
                           "\"wal_level\" to be \"replica\" or \"logical\"")));
  ...
  if (ListenAddresses) {
    ...
    foreach (l, elemlist) {
      ...
      if (strcmp(curhost, "*") == 0)
        status =
            ListenServerPort(AF_UNSPEC, NULL, (unsigned short) PostPortNumber,
                             NULL, ListenSockets, &NumListenSockets, MAXLISTEN);
      else
        status = ListenServerPort(AF_UNSPEC, curhost,
                                  (unsigned short) PostPortNumber, NULL,
  ...
  if (enable_bonjour && NumListenSockets > 0) {
    ...
    err = DNSServiceRegister(&bonjour_sdref, 0, 0, bonjour_name,
                             ...
                             pg_hton16(PostPortNumber), 0, NULL, NULL, NULL);
    if (err != kDNSServiceErr_NoError)
      ereport(LOG, (errmsg("DNSServiceRegister() failed: error code %ld",
                           (long) err)));
  ...
  if (Unix_socket_directories) {
    ...
    foreach (l, elemlist) {
      ...
      status = ListenServerPort(AF_UNIX, NULL, (unsigned short) PostPortNumber,
                                socketdir, ListenSockets, &NumListenSockets,
                                MAXLISTEN);
   ...
   */
  ereport(LOG, (errmsg("start startup process in main process %d", getpid())));
  ...
  ereport(LOG, (errmsg("Before ServerLoop")));
  status = ServerLoop();
  ereport(LOG, (errmsg("After ServerLoop")));
  ...
  if (pdir == NULL)
    ereport(
        ...
        (errcode_for_file_access(),
         ...
         errhint(
             "This may indicate an incomplete PostgreSQL installation, or that "
...
static void ConfigurePostmasterWaitSet(bool accept_connections) {
  ...
  if (accept_connections) {
    for (int i = 0; i < NumListenSockets; i++)
      AddWaitEventToSet(pm_wait_set, WL_SOCKET_ACCEPT, ListenSockets[i], NULL,
                        NULL);
  }
}
...
static int ServerLoop(void) {
  ...
  for (;;) {
     ...
     */
    ereport(LOG, (errmsg("Got %d events", nevents)));
    // FLEX: If got events 0, we think the postmaster has finished
    // initialization.
    for (int i = 0; i < nevents; i++) {
      // printf("Got event %d\n", events[i].events);
      ereport(LOG, (errmsg("Got event %d", events[i].events)));
      if (events[i].events & WL_LATCH_SET) {
        ereport(LOG, (errmsg("Got WL_LATCH_SET")));
        ResetLatch(MyLatch);
      }
    ...
    if (now - last_lockfile_recheck_time >= 1 * SECS_PER_MINUTE) {
      if (!RecheckDataDirLockFile()) {
        ereport(LOG, (errmsg("performing immediate shutdown because data "
...
void ClosePostmasterPorts(bool am_syslogger) {
  ...
  if (close(postmaster_alive_fds[POSTMASTER_FD_OWN]) != 0)
    ereport(FATAL, (errcode_for_file_access(),
                    errmsg_internal("could not close postmaster death "
                                    "monitoring pipe in child process: %m")));
 ...
 */
static void handle_pm_shutdown_request_signal(SIGNAL_ARGS) {
  ereport(LOG, (errmsg_internal("postmaster received signal to stop")));
...
static void process_pm_shutdown_request(void) {
  ...
  case SmartShutdown:
    ...
    if (pmState == PM_RUN || pmState == PM_HOT_STANDBY)
      connsAllowed = false;
    else if (pmState == PM_STARTUP || pmState == PM_FLEX ||
             pmState == PM_RECOVERY) {
  ...
  case FastShutdown:
    ...
    if (pmState == PM_STARTUP || pmState == PM_FLEX || pmState == PM_RECOVERY) {
...
static void process_pm_child_exit(void) {
  ...
  while ((pid = waitpid(-1, &exitstatus, WNOHANG)) > 0) {
    ...
    if (pid == StartupPID) {
      ...
      if (pmState == PM_FLEX && FlexStatus != STARTUP_SIGNALED &&
          !EXIT_STATUS_0(exitstatus)) {
      ...
      if (!EXIT_STATUS_0(exitstatus)) {
        if (StartupStatus == STARTUP_SIGNALED) {
          StartupStatus = STARTUP_NOT_RUNNING;
          if (pmState == PM_STARTUP || pmState == PM_FLEX)
            pmState = PM_WAIT_BACKENDS;
 ...
 */
static void CleanupBackend(Backend* bp,
                           int exitstatus) /* child's exit status. */
{
  ...
  if (bp->bkend_type == BACKEND_TYPE_BGWORKER) {
    ...
    if (!logged) {
      LogChildExit(EXIT_STATUS_0(exitstatus) ? DEBUG1 : LOG, procname, bp->pid,
                   exitstatus);
      logged = true;
    }
...
static void HandleChildCrash(int pid, int exitstatus, const char* procname) {
  ...
  if (pmState == PM_RECOVERY || pmState == PM_HOT_STANDBY ||
      pmState == PM_RUN || pmState == PM_STOP_BACKENDS ||
      pmState == PM_SHUTDOWN)
    pmState = PM_WAIT_BACKENDS;
...
static void PostmasterStateMachine(void) {
  ...
  if (pmState == PM_WAIT_BACKENDS) {
    ...
    if (CountChildren(BACKEND_TYPE_ALL - BACKEND_TYPE_WALSND) == 0 &&
        StartupPID == 0 && WalReceiverPID == 0 && WalSummarizerPID == 0 &&
        BgWriterPID == 0 &&
        (CheckpointerPID == 0 ||
         (!FatalError && Shutdown < ImmediateShutdown)) &&
        WalWriterPID == 0 && AutoVacPID == 0 && SlotSyncWorkerPID == 0) {
...
static void LaunchMissingBackgroundProcesses(void) {
  ...
  if (WalReceiverRequested) {
    if (WalReceiverPID == 0 &&
        (pmState == PM_STARTUP || pmState == PM_FLEX ||
         pmState == PM_RECOVERY || pmState == PM_HOT_STANDBY) &&
        Shutdown <= SmartShutdown) {
...
static int BackendStartup(ClientSocket* client_sock) {
  ...
  pid = postmaster_child_launch(B_BACKEND, (char*) &startup_data,
                                sizeof(startup_data), client_sock);
  if (pid < 0) {
...
static void process_pm_pmsignal(void) {
  ...
  if (CheckPostmasterSignal(PMSIGNAL_RECOVERY_STARTED) &&
      (pmState == PM_STARTUP || pmState == PM_FLEX) && Shutdown == NoShutdown) {
...
int MaxLivePostmasterChildren(void) {
  return 2 * (MaxConnections + autovacuum_max_workers + 1 + max_wal_senders +
...
static void WINAPI pgwin32_deadchild_callback(PVOID lpParameter,
   ...
   * from this thread.
   */
  if (!PostQueuedCompletionStatus(win32ChildQueue, 0, (ULONG_PTR) lpParameter,
                                  NULL))
...
void pgwin32_register_deadchild_callback(HANDLE procHandle, DWORD procId) {
  ...
  if (!RegisterWaitForSingleObject(
          &childinfo->waitHandle, procHandle, pgwin32_deadchild_callback,
...
static void InitPostmasterDeathWatchHandle(void) {
  ...
  if (fcntl(postmaster_alive_fds[POSTMASTER_FD_WATCH], F_SETFL, O_NONBLOCK) ==
    ...
    ereport(FATAL, (errcode_for_socket_access(),
                    errmsg_internal("could not set postmaster death monitoring "
