...
void StartupProcessMain(char* startup_data, size_t startup_data_len) {
   ...
   * successfully.
   */
  ereport(LOG, (errmsg("startup process (PID %d) exited with exit code 0",
                       getpid())));
  proc_exit(0);
}
...
void PostRestoreCommand(void) { in_restore_command = false; }

bool IsPromoteSignaled(void) { return promote_signaled; }

void ResetPromoteSignaled(void) { promote_signaled = false; }
...
bool has_startup_progress_timeout_expired(long* secs, int* usecs) {
  ...
  TimestampDifference(startup_progress_phase_start_time, now, &seconds,
                      &useconds);
