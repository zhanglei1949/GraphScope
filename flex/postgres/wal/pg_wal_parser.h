...
#ifndef ENGINES_GRAPH_DB_DATABASE_WAL_PG_WAL_PARSER_H_
#define ENGINES_GRAPH_DB_DATABASE_WAL_PG_WAL_PARSER_H_
...
// TODO: Implement the real wal parser for postgres wal
class PGWalParser : public IWalParser {
 public:
 static void Init();
  static std::unique_ptr<IWalParser> Make(const std::string& wal_dir) {
    return std::unique_ptr<IWalParser>(new PGWalParser(wal_dir));
  }

  PGWalParser(const std::string& wal_dir);
  ~PGWalParser();
...
#endif  // ENGINES_GRAPH_DB_DATABASE_WAL_PG_WAL_PARSER_H_