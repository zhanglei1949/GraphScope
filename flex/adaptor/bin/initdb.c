...
//static const char *const backend_options = "-F -O -j -c search_path=pg_catalog -c exit_on_error=true -c log_checkpoints=false";
...
bootstrap_template1(void)
{
	...
	printf("before PG_CMD_CLOSE\n");
	PG_CMD_CLOSE();

	printf("before termPQExpBuffer\n");
	termPQExpBuffer(&cmd);
	free(bki_lines);

	printf("before check_ok\n");
	check_ok();
	printf("Finished bootstrap template\n");
}
...
{
	printf("enter setup_bin_paths\n");
	int			ret;

	if ((ret = find_other_exec(argv0, "flex_adaptor", PG_BACKEND_VERSIONSTR,
							   backend_exec)) < 0)
	{
		char		full_path[MAXPGPATH];

		printf("progname: %s, ret %d\n", progname, ret);
		if (find_my_exec(argv0, full_path) < 0){
			printf("find_my_exec failed\n");
	...
	*last_dir_separator(bin_path) = '\0';
	...
	if (!share_path)
	{
		...
		share_path = "/opt/postgres/share"; // TODO: FIXME
	}
	...
	printf("after canonicalize_path share_path: %s\n", share_path);
}
...
{
	printf("Initializing database cluster\n");
	PG_CMD_DECL;
	PQExpBufferData cmd;
	int			i;
	...
	printf("2 finish setup_run_file\n");
	...
	check_ok();
	printf("Finish initializing database cluster\n");
}
