...
class HouYan : public WriteAppBase {
 public:
...
#define DEBUG
  bool Query(GraphDBSession& graph, Decoder& input, Encoder& output) {
    ...
    auto person_invest_cmp_outgoing_view =
        txn.GetOutgoingImmutableGraphView<RecordView>(
            person_label_id_, company_label_id_, person_invest_cmp_label_id_);
    auto person_invest_cmp_incoming_view =
        txn.GetIncomingImmutableGraphView<RecordView>(
            company_label_id_, person_label_id_, person_invest_cmp_label_id_);
 ...
 private:
  ...
  label_t person_invest_cmp_label_id_;
...
void* CreateApp(gs::GraphDBSession& db) {
  gs::HouYan* app = new gs::HouYan();
  return static_cast<void*>(app);
}

void DeleteApp(void* app) {
  gs::HouYan* casted = static_cast<gs::HouYan*>(app);
  delete casted;
}
}
