...
void GraphDBUpdateService::init(uint32_t num_shards, uint16_t http_port,
                                bool dpdk_mode) {
  actor_sys_ = std::make_unique<actor_system>(num_shards, dpdk_mode);
  http_hdl_ = std::make_unique<graph_db_update_http_handler>(http_port);
}

void GraphDBUpdateService::run_and_wait_for_exit() {
...
void GraphDBUpdateService::set_exit_state() { running_.store(false); }
