 ...
 public:
  ...
  seastar::future<std::unique_ptr<seastar::httpd::reply>> handle(
    ...
    return executor_refs_[dst_executor]
        .run_query(::server::query_param{std::move(req->content)})
        .then_wrapped(
            [rep = std::move(rep)](
                seastar::future<::server::query_result>&& fut) mutable {
...
class adhoc_query_handler : public seastar::httpd::handler_base {
 public:
  ...
  seastar::future<std::unique_ptr<seastar::httpd::reply>> handle(
    ...
    return executor_refs_[dst_executor]
        .run_adhoc_query(::server::query_param{std::move(req->content)})
        .then_wrapped(
            [rep = std::move(rep)](
                seastar::future<::server::query_result>&& fut) mutable {
              if (__builtin_expect(fut.failed(), false)) {
...
class exit_handler : public seastar::httpd::handler_base {
 public:
  seastar::future<std::unique_ptr<seastar::httpd::reply>> handle(
      ...
      std::unique_ptr<seastar::httpd::request> req,
      std::unique_ptr<seastar::httpd::reply> rep) override {
    hqps_server::service::get().set_exit_state();
    rep->write_body(
        "bin",
