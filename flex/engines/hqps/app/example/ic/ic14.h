...
class QueryIC14 {
  ...
  void get_post_person_nbrs(
    ...
    auto comment_nbr_list_array = graph.GetOtherVertices(
        SNBLabels::comment_label, SNBLabels::person_label,
        SNBLabels::hasCreator_label, src_vec, "In", INT_MAX);
    ...
    auto post_nbr_list_array = graph.GetOtherVertices(
        SNBLabels::comment_label, SNBLabels::post_label,
        SNBLabels::replyOf_label, cmt_ids, "Out", INT_MAX);
    ...
    auto person_nbr_list_array = graph.GetOtherVertices(
        SNBLabels::post_label, SNBLabels::person_label,
        SNBLabels::hasCreator_label, post_ids, "Out", INT_MAX);
  ...
  void get_cmt_person_nbrs(
    ...
    //
    auto post_nbr_list_array = graph.GetOtherVertices(
        SNBLabels::comment_label, SNBLabels::comment_label,
        SNBLabels::replyOf_label, cmt_ids, "Out", INT_MAX);
    std::vector<vertex_id_t> post_ids;
    for (auto i = 0; i < post_nbr_list_array.size(); ++i) {
      for (auto nbr : post_nbr_list_array.get(i)) {
 ...
 public:
  ...
  void Query(const GRAPH_INTERFACE& graph, int64_t ts, Decoder& input,
    ...
    for (auto i = 0; i < inds.size(); ++i) {
      ...
      auto oids = graph.template GetVertexPropsFromVid<oid_t>(person_label_id,
                                                              vec, props);
