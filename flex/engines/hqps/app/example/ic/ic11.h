...
class QueryIC11 {
 ...
 public:
  ...
  void Query(const GRAPH_INTERFACE& graph, int64_t ts, Decoder& input,
    ...
    label_id_t is_locatedIn_label_id =
    ...
    auto ctx6 = Engine::Sort(
        ...
        std::tuple{
            gs::OrderingPropPair<gs::SortOrder::ASC, INPUT_COL_ID(1), int32_t>(
                "none"),
            gs::OrderingPropPair<gs::SortOrder::ASC, INPUT_COL_ID(0), int64_t>(
                "id"),
            gs::OrderingPropPair<gs::SortOrder::DESC, INPUT_COL_ID(2),
                                 std::string_view>("name")});

    auto ctx7 = Engine::template Project<INPUT_COL_ID(0)>(
        graph, std::move(ctx6),
