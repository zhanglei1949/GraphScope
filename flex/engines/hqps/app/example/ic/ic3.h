...
class QueryIC3 {
 ...
 public:
  ...
  void Query(const GRAPH_INTERFACE& graph, int64_t ts, Decoder& input,
    ...
    label_id_t isPartOf_label_id =
        graph.GetEdgeLabelId(SNBLabels::isPartOf_label);
    ...
    auto ctx_y4 = Engine::GroupBy(
        ...
        std::tuple{gs::make_aggregate_prop<gs::AggFunc::COUNT>(
            std::tuple{gs::PropertySelector<grape::EmptyType>()},
            std::integer_sequence<int32_t, INPUT_COL_ID(0)>{})});

    auto ctx_joined = Engine::template Join<INPUT_COL_ID(0), INPUT_COL_ID(0),
                                            JoinKind::InnerJoin>(
