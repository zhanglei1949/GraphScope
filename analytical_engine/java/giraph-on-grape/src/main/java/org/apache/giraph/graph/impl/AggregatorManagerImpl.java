...
//    private HashMap<String,Aggregator> unPersistentAggregators;
    ...
    @Override
    public void setFFICommunicator(FFICommunicator communicator){
        ...
        for (Entry<String, AggregatorWrapper<Writable>> entry :
            ...
            try {
                ...
                while (inputStream.longAvailable() > 0){
                    ...
                    wrapper.reduce(msg);
                    logger.info("worker: " + workerId + "aggregator: " + aggregatorKey + " reduce: " + msg + ", to" + wrapper.getCurrentValue());
                }
                logger.info("worker: " + workerId +  "aggregator: " + aggregatorKey + " after aggregation: " + wrapper.getCurrentValue());
            }
            catch (IOException e) {
                e.printStackTrace();
            }
...
//        initAggregatorValues.clear();
    ...
    private <A extends Writable> boolean  registerAggregator(String name, Class<? extends Aggregator<A>> aggregatorClass, boolean persistent){
