...
public class LdbcTest {
                ...
                new Configs(
                        ImmutableMap.of(
                                ...
                                "NotMatchToAntiJoinRule, FilterIntoJoinRule, FilterMatchRule,"
                                        + " FlatJoinToExpandRule, FlatJoinToCommonRule,"
    ...
    public void ldbc9_2_test() {
        ...
        Assert.assertEquals(
                "GraphLogicalSort(sort0=[commentOrPostCreationDate], sort1=[commentOrPostId],"
                    ...
                    + " isAppend=[false])\n"
                    + "      GraphPhysicalExpand(tableConfig=[{isAll=false, tables=[HASCREATOR]}],"
                    + " alias=[message], startAlias=[friend], fusedFilter=[[<(_.creationDate,"
                    + " ?1)]], opt=[IN], physicalOpt=[VERTEX])\n"
                    + "        LogicalFilter(condition=[<>(friend.id, ?0)])\n"
                    + "          GraphLogicalAggregate(keys=[{variables=[friend],"
                    + " aliases=[friend]}], values=[[]])\n"
                    + "            GraphLogicalGetV(tableConfig=[{isAll=false, tables=[PERSON]}],"
    ...
    public void ldbc10_2_test() {
        ...
        RelNode before =
                com.alibaba.graphscope.cypher.antlr4.Utils.eval(
                                "MATCH (person:PERSON {id: $personId})-[:KNOWS*2..3]-(friend:"
                                        ...
                                        +
                                        //                                        "WHERE
                                        // (birthday.month=$month AND birthday.day>=21) OR
                                        // (birthday.month=(($month + 0)%12) + 1 AND
                                        // birthday.day<22)\n" +
                                        "WITH DISTINCT friend\n"
                                      + "\n"
                                      ...
                                      + "        MATCH (friend: PERSON) <- [:HASCREATOR]- (post:"
                                      + " POST) - [:HASTAG] -> (tag:TAG)\n"
                                      + "        WITH friend, postCount, post, tag\n"
                                      + "        MATCH (tag:TAG)<-[:HASINTEREST]-(person: PERSON"
                                      + " {id: $personId})\n"
                                      + "        WITH friend, postCount, count(distinct post) as"
                                      + " commonPostCount\n"
                                      + "        return friend, commonPostCount - (postCount -"
                                      + " commonPostCount) AS commonInterestScore, friend.id as"
                                      + " friendId\n"
                                      ...
                                      + "UNION \n"
                                      + "CALL {\n"
                                      + "        return friend, (0L-postCount) AS"
                                      + " commonInterestScore, friend.id as friendId\n"
                                      + "        ORDER BY commonInterestScore DESC, friendId ASC\n"
                                      + "        LIMIT 10\n"
                                      + "}\n"
                                      + "WITH friend, commonInterestScore, friendId\n"
                                      + "ORDER BY commonInterestScore DESC, friendId ASC\n"
                                      + "LIMIT 10\n"
        ...
        Assert.assertEquals(
                "root:\n"
                    + "GraphLogicalProject(friendId=[friendId], personFirstName=[friend.firstName],"
                    + " personLastName=[friend.lastName],"
                    + " commonInterestScore=[commonInterestScore], personGender=[friend.gender],"
                    + " personCityName=[city.name], isAppend=[false])\n"
                    + "  GraphPhysicalExpand(tableConfig=[[EdgeLabel(ISLOCATEDIN, PERSON, PLACE)]],"
                    + " alias=[city], startAlias=[friend], opt=[OUT], physicalOpt=[VERTEX])\n"
                    + "    GraphLogicalSort(sort0=[commonInterestScore], sort1=[friendId],"
                    + " dir0=[DESC], dir1=[ASC], fetch=[10])\n"
                    + "      GraphLogicalProject(friend=[friend],"
                    + " commonInterestScore=[commonInterestScore], friendId=[friendId],"
                    + " isAppend=[false])\n"
                    + "        LogicalUnion(all=[true])\n"
                    + "          GraphLogicalSort(sort0=[commonInterestScore], sort1=[friendId],"
                    + " dir0=[DESC], dir1=[ASC], fetch=[10])\n"
                    ...
                    + " friendId=[friend.id], isAppend=[false])\n"
                    + "              GraphLogicalAggregate(keys=[{variables=[friend, postCount],"
                    + " aliases=[friend, postCount]}], values=[[{operands=[post],"
                    + " aggFunction=COUNT, alias='commonPostCount', distinct=true}]])\n"
                    + "                LogicalJoin(condition=[=(tag, tag)], joinType=[inner])\n"
                    + "                  GraphLogicalProject(friend=[friend],"
                    + " postCount=[postCount], post=[post], tag=[tag], isAppend=[false])\n"
                    + "                    GraphPhysicalExpand(tableConfig=[[EdgeLabel(HASTAG,"
                    + " POST, TAG)]], alias=[tag], opt=[OUT], physicalOpt=[VERTEX])\n"
                    + "                      GraphPhysicalGetV(tableConfig=[{isAll=false,"
                    + " tables=[POST]}], alias=[post], opt=[START], physicalOpt=[ITSELF])\n"
                    + "                       "
                    ...
                    + " alias=[_], startAlias=[friend], opt=[IN], physicalOpt=[VERTEX])\n"
                    + "                          GraphLogicalProject(friend=[friend],"
                    + " postCount=[postCount], isAppend=[false])\n"
                    + "                            CommonTableScan(table=[[common#-1903810612]])\n"
                    + "                  GraphPhysicalExpand(tableConfig=[{isAll=false,"
                    ...
                    + "          GraphLogicalSort(sort0=[commonInterestScore], sort1=[friendId],"
                    + " dir0=[DESC], dir1=[ASC], fetch=[10])\n"
                    + "            GraphLogicalProject(friend=[friend], commonInterestScore=[-(0,"
                    + " postCount)], friendId=[friend.id], isAppend=[false])\n"
                    + "              CommonTableScan(table=[[common#-1903810612]])\n"
                    + "common#-1903810612:\n"
                    + "GraphLogicalProject(friend=[friend], postCount=[postCount],"
                    + " isAppend=[false])\n"
                    + "  GraphPhysicalExpand(tableConfig=[[EdgeLabel(HASCREATOR, POST, PERSON)]],"
                    + " alias=[postCount], startAlias=[friend], opt=[IN], physicalOpt=[DEGREE],"
                    + " optional=[true])\n"
                    + "    GraphLogicalAggregate(keys=[{variables=[friend], aliases=[friend]}],"
                    + " values=[[]])\n"
                    + "      GraphLogicalProject(person=[person], friend=[friend],"
                    + " birthday=[friend.birthday], isAppend=[false])\n"
    ...
    public void ldbc14_test() {
        ...
        Assert.assertEquals(
                "root:\n"
                    ...
                    + "            LogicalJoin(condition=[AND(=(rel0, rel0), =(rel1, rel1))],"
                    + " joinType=[left])\n"
                    + "              CommonTableScan(table=[[common#-1230129050]])\n"
                    + "              GraphPhysicalExpand(tableConfig=[[EdgeLabel(HASCREATOR, POST,"
                    + " PERSON), EdgeLabel(HASCREATOR, COMMENT, PERSON)]], alias=[rel1], opt=[OUT],"
                    + " physicalOpt=[VERTEX])\n"
                    + "                GraphPhysicalExpand(tableConfig=[[EdgeLabel(REPLYOF,"
                    + " COMMENT, POST), EdgeLabel(REPLYOF, COMMENT, COMMENT)]], alias=[m],"
                    + " opt=[BOTH], physicalOpt=[VERTEX])\n"
                    + "                  GraphPhysicalExpand(tableConfig=[{isAll=false,"
                    + " tables=[HASCREATOR]}], alias=[n], startAlias=[rel0], opt=[IN],"
                    + " physicalOpt=[VERTEX])\n"
                    + "                    GraphLogicalAggregate(keys=[{variables=[rel0],"
                    + " aliases=[rel0]}], values=[[]])\n"
                    + "                      CommonTableScan(table=[[common#-1230129050]])\n"
                    + "common#-1230129050:\n"
